...
def m():
    while True:
        for s in list(players.keys()): # перебираем всех игроков кто подключился
            try:
                data = s.recv(1024) # получаем данные от клиента
                if data:
                    parts = data.decode().strip("|").split(",") # розбиваем строку
                    if len(parts) == 3:
                        x, y, name = parts
                        players[s]["x"] = int(x) # обновляем координаты
                        players[s]["y"] = int(y)
                        players[s]["name"] = name
            except:
                pass
            ...
            for key, valuer in players.items():
                    if key != s :
                        line = f"{valuer['id']},{valuer['x']},{valuer['y']},{valuer['r']},{valuer['name']}"
                        pocket +=line + "|"                        
